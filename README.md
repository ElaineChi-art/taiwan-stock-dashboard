...
## 功能
- **即時行情圖（自建）**：用開源的 Lightweight Charts 畫互動圖，可切換標的。
  - 加密貨幣：瀏覽器直接打 Binance 公開 API → **真即時**。
  - 台股：由 `intraday.py` 在盤中每 15 分鐘抓 5 分 K 存成 `docs/data/*.json`，前端讀取 → **約延遲 15 分鐘**（免費資料上限）。
...
- **自動更新**：`daily.yml` 每日算 AI 預測；`intraday.yml` 盤中每 15 分更新台股即時資料。
